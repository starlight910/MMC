...
void addsyn(short sData[],	// array for the resulting sound
	...
	int nPartials,		// number of partials (length of arrays)
	int pd)
{
	...
	void createPartial(double tone[], double F, double A, double modF, double modF2,  double dur, int sr, int
			pd);
	...
	for ( n = 0 ; n < nPartials ; n++ ) {	// loop to create partials
		...
		createPartial(tone, f, A, modF, modF2, dur, sr, pd) ;		// create one partial
	}
		...
		int pd)
{
	...
	modPhInc = (2.0 * M_PI * modF) / sr ;
	modPhInc2 = (2.0 * M_PI * modF2) / sr ;
	samples = dur * sr ;				// duration of sound in samples

	for ( t = 0 ; t < samples ; t++ ) {	// create the partial
		newamp = A * sin(ph + pd * sin(ph2 + pd2 * sin(ph3))) ;			// calculate new amplitude
